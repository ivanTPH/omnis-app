...
export default function EhcpPageClient({ plans, studentsWithoutEhcp, isSenco }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Plans awaiting SENCO sign-off banner */}
      {pendingApproval.length > 0 && isSenco && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
          <Icon name="pending" size="sm" className="text-amber-600 shrink-0 mt-0.5" />
          <p className="text-sm text-amber-800">
            <span className="font-semibold">{pendingApproval.length} EHCP plan{pendingApproval.length > 1 ? 's' : ''}</span>
            {' '}awaiting your review and sign-off. Review each plan below, add commentary, then approve to activate.
          </p>
        </div>
      )}
      ...
      {viewMode === 'no_ehcp' ? (
        ...
        <div className="space-y-0">
          {filteredPlans.map(plan => {
            ...
            const avatarColour = isOverdue ? 'bg-red-400' : isDueSoon ? 'bg-amber-400' : 'bg-purple-400'
            ...
            const badges = [
              ...
              isAiDraft
                ? { label: 'Needs approval', variant: 'draft' as const }

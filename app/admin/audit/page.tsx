...
function buildHref(params: { page?: number; category?: string; days?: number }) {
...
export default async function AdminAuditPage({
  ...
  const category = sp.category && AUDIT_CATEGORIES[sp.category] ? sp.category : undefined
          ...
          <div className="flex flex-wrap gap-1.5">
            {CATEGORY_CHIPS.map(chip => {
              ...
              return (
                <Link
                  key={chip.label}
                  href={buildHref({ category: chip.key, days })}
          ...
          <div className="flex gap-1.5">
            {DATE_CHIPS.map(chip => {
              ...
              return (
                <Link
                  key={chip.label}
                  href={buildHref({ category, days: chip.days })}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4">
            ...
            <div className="flex gap-2">
              {page > 0 && (
                <Link
                  href={buildHref({ page: page - 1, category, days })}

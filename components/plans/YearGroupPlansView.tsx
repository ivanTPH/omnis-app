...
const EDIT_ROLES    = ['HEAD_OF_DEPT', 'HEAD_OF_YEAR', 'SLT', 'SCHOOL_ADMIN']
...
export default function YearGroupPlansView({
  ...
  // Derive subjects from existing plans + ALL_SUBJECTS for the tab list
  const subjects = useMemo(() => {
    const fromPlans = plans.map(p => p.subject)
    const merged    = Array.from(new Set([...fromPlans, ...ALL_SUBJECTS]))
    return merged.sort()
  }, [plans])
  ...
  // Plans indexed by subject→yearGroup for quick lookup
  ...
  function openCreate(subject: string, yearGroup: number) {
    ...
    if (existing) {
      setModal({
        ...
        yearGroup,
        subject,
        content:         existing.planContent,
        fileUrl:         existing.uploadedFileUrl ?? '',
        submitAfterSave: false,
      })
    } else {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (modal.submitAfterSave && modal.plan?.id) {
        await submitForApproval(modal.plan.id)
        toast('Plan saved and submitted for approval')
      } else if (modal.submitAfterSave) {
        // After upsert the plan exists — re-fetch is handled by router.refresh()
        toast('Plan saved — submit it for approval from the list')
      } else {
        toast('Plan saved')
      }
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-5">
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Scheme of Work — view and manage curriculum plans by subject and year group</p>
          </div>
          {canEdit && (
            <button
              ...
              {orderedSubjects.map(subject => {
                ...
                return (
                  ...
                  >
                    ...
                    {hasPlan && (
                      <span className="shrink-0 ml-1 w-2 h-2 rounded-full bg-blue-400" />
                    )}
                  </button>
                )
              })}
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2 justify-end" onClick={e => e.stopPropagation()}>
                          {canEdit && (
                            ...
                            >
                              <Icon name={plan ? 'edit' : 'add'} size="sm" />
                              {plan ? 'Edit' : 'Create'}
                            </button>
                          )}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={14}
                  placeholder={`Describe the scheme of work for ${modal.subject || 'this subject'} Year ${modal.yearGroup}...\n\n## Term 1\n- Topic 1\n- Topic 2\n\n## Term 2\n- Topic 3`}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  Uploaded File URL
                  <span className="text-gray-400 font-normal ml-1">(optional — link to PDF or document)</span>
                </label>
                <input
                  type="url"
                  value={modal.fileUrl}
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                />
                <span className="text-sm text-gray-700">Submit for approval after saving</span>
              </label>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200">
              ...
              >
                {saving
                  ...
                  : <><Icon name="save" size="sm" />Save Plan</>
                }
              </button>
            </div>
          </div>

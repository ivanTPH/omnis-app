  ...
  const concerns = await Promise.all([
    ...
    prisma.sendConcern.create({
      data: {
        ...
        source: 'teacher',
        category: 'literacy',
        description: `${s2.firstName} is producing significantly less written work than peers. Extended writing tasks appear to trigger anxiety responses — task avoidance and incomplete submissions are a recurring pattern. Verbal contributions suggest genuine understanding, but this is not translating into written output. SEMH needs are clearly impacting engagement with independent written tasks.`,
        status: 'open',
    ...
    s3 ? prisma.sendConcern.create({
      data: {
        ...
        description: `${s3.firstName} demonstrates significant difficulties with verbal communication. Frequently misunderstands spoken instructions and struggles to express ideas clearly in class discussions.`,
        status: 'monitoring',
        reviewedBy: senco.id,
    ...
    s4 ? prisma.sendConcern.create({
      data: {
        ...
        source: 'teacher',
        category: 'behaviour',
        description: `${s4.firstName} has been disruptive in 5 of the last 8 lessons. Difficulty remaining on task and frequently distracts other students. Possible attention/concentration difficulties.`,
        status: 'closed',
        reviewedBy: senco.id,
